...
class ExternalOptimizerInterface(object):
  ...
  def _make_eval_func(self, tensors, session, feed_dict, fetches,
    ...
    def eval_func(x):
      ...
      fetches = callback.merged_summaries
      augmented_feed_dict = {
      ...
      augmented_fetch_vals = session.run(
        ...
        options=callback.run_options, run_metadata=callback.run_metadata)
...
class ITLOptimizerInterface(ExternalOptimizerInterface):
  _DEFAULT_METHOD = 'NQN-L-BFGS-B'

  def _minimize(self, initial_val, loss_grad_func, equality_funcs,
                equality_grad_funcs, inequality_funcs, inequality_grad_funcs,
                packed_bounds, step_callback, optimizer_kwargs, run_kwargs):
    ...
    if method == 'NQN-L-BFGS-B':
      from .nqn import fmin_l_bfgs_b
      result = fmin_l_bfgs_b(funObj=lambda x: minimize_args[0](x)[0],
                             x=minimize_args[1],
                             gradObj=lambda x: minimize_args[0](x)[1],
                             bounds=minimize_kwargs['bounds'],
                             callback=minimize_kwargs['callback'],
