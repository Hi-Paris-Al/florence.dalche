...
def fmin_l_bfgs_b(funObj, x, gradObj, bounds=None, m=20, M=1, pgtol=1e-5,
                  iprint=-1, maxfun=15000, maxiter=15000, callback=None,
                  factr=0., **kwargs):
    '''
    Termination Flag:
    0 => Converged
    ...
    if 'gtol' in kwargs:
        pgtol = kwargs['gtol']
    if 'ftol' in kwargs:
        factr = kwargs['ftol'] / numpy.finfo(float).eps
    if 'maxcor' in kwargs:
        m = kwargs['maxcor']
    ...
    for it in range(maxiter):
        ...
        if(iprint > -1):
            print('Iteration', it, 'NFE', func_calls, 'Function Value:', f,
                  '||g||', numpy.linalg.norm(pg, numpy.inf),
                  'Active', len(bnds.activity(x, grad_samp_grad)),
                  'AtBounds', len(bnds.atbounds(x)), 'alpha', alpha)
    ...
    return_dict = {}
    return_dict['success'] = (termination_flag == 0)
    return_dict['message'] = termination_flag
    return_dict['grad'] = g
    return_dict['jac'] = g
    return_dict['nfev'] = func_calls
    return_dict['nit'] = it
    return_dict['x'] = x
    return_dict['fun'] = f
    if return_hist:
        return_dict['history'] = numpy.array(fga_hist)
    return OptimizeResult(return_dict)
...
def main():
    ...
    print('SciPy:',
          scipy.optimize.fmin_l_bfgs_b(func, x0, fprime=grad,
                                       bounds=bounds)[:2])
    print('NQN (base):',
          fmin_l_bfgs_b(func, x0, grad, bounds))
    print('NQN (BFGS, m=Inf):',
          fmin_l_bfgs_b(func, x0, grad, bounds, m=numpy.inf))
    print('NQN (M=20):',
          fmin_l_bfgs_b(func, x0, grad, bounds, M=20))
    print('NQN (callback):',
          fmin_l_bfgs_b(func, x0, grad, bounds,
                        callback=lambda x: numpy.save('TEST', x)))
    return 0
