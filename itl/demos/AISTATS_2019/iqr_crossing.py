...
def main(argv=None):
    ...
    for i, kappa in enumerate([10 * N, 0]):
        if path != '':
            graph = path + ('/' + str(kappa))
        else:
            graph = path
        h = ITLModel(Gaussian(10), Gaussian(10), Gaussian(10),
                     derivative=not np.isclose(kappa, 0))
        est = QuantileReg(model=h, solver='L-BFGS-B',
                          ...
                          sampler=GaussLegendreUniform_0p1(20),
        ...
        if kappa > 0:
            axis[i].set_title(r'Non-crossing: '
                              r'$\lambda_{{nc}}={}$'.format(
                kappa / N))
        else:
            axis[i].set_title(r'Crossing: $\lambda_{{nc}}=0$')
