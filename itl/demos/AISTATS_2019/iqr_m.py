...
def main(argv=None):
    ...
    if path != '':
        ...
        graph = path
    for i, m in enumerate(np.linspace(2, 100, 4)):
        ax = axis[i // 2, i % 2]
        est = QuantileReg(model=ITLModel(Gaussian(1),
                          ...
                          solver_param={'disp': 1},
                          sampler=SobolUniform_0p1(int(m)),
                          cost=cost.HuberPinball(kappa=0),
                          summary={'path': graph})
    ...
    if show:
        ...
        if not os.path.exists(prefix):
            os.mkdir(prefix)
        fig.savefig(prefix + '/iqr_m.eps', bbox_inches='tight')
        fig.savefig(prefix + '/iqr_m.pdf', bbox_inches='tight')
